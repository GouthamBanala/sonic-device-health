...
class AristaSwitchCLIHelper(object):
    ...
    def execute_arista_command(self, command, option='show', privilege_level=15, print_output=False):
        """
    ...
    def is_unix_eapi_running(self):
        ...
        try:
            output = self.execute_arista_command(arista_command, option='show', print_output=False)
    ...
    def enable_unix_eAPI_protocol(self):
        ...
        arista_command = "configure\n\
                          ...
                          no shutdown"
        try:
            self.execute_arista_command(arista_command, option='config', print_output=False)
        except subprocess.CalledProcessError as e:
            raise
    ...
    def check_and_enable_unix_eAPI_protocol(self):
        ...
        try:
            ...
            if not running:
                ...
                time.sleep(api.CLI_CONFGURATION_WAIT_TIME)  # Wait for the configuration to take effect
    ...
    def get_event_handler_data(self, handler_name):
        ...
        try:
            output = self.execute_arista_command(arista_command, option='config', print_output=False)
    ...
    def commit_config(self):
        ...
        arista_command = "configure\n\
                          write memory"
        try:
            result = self.execute_arista_command(arista_command, option='config', print_output=False)
            return result 
        except subprocess.CalledProcessError as e:
            raise
